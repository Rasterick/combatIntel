<?php
    ...
    curl_setopt($ch, CURLOPT_HTTPHEADER, [
        ...
        'User-Agent: combatIntel (abonriff@gmail.com, https:
     //gpi-services.co.uk/combatIntel/combatintel.html)'
    ]);
    ...
    if ($latestKillData === FALSE || $httpCode !== 200) {
        ...
        error_log("Failed to fetch latest kill summaries from zKillboard. HTTP Code: " . $httpCode . ",
    Response: " . $latestKillData);
    ...
    if (is_array($latestKillSummaries) && !empty($latestKillSummaries)) {
            ...
            if ($latestKillId && $latestKillHash) {
                $fullKillmailApiUrl = "https://esi.evetech.net/latest/killmails/{$latestKillId}/{
       $latestKillHash}/?datasource=tranquility";
    ...
    if (isset($zkbStats['info'])) {
        ...
        if (isset($zkbStats['info']['corporation_id'])) $idsToResolve[] = $zkbStats['info'][
     'corporation_id'];
    ...
    // IDs from latestKill
    if ($latestKill) {
        if (isset($latestKill['victim']['character_id'])) $idsToResolve[] = $latestKill['victim'][
       'character_id'];
        if (isset($latestKill['victim']['corporation_id'])) $idsToResolve[] = $latestKill['victim'][
     'corporation_id'];
        if (isset($latestKill['victim']['alliance_id'])) $idsToResolve[] = $latestKill['victim'][
      'alliance_id'];
        if (isset($latestKill['victim']['ship_type_id'])) $idsToResolve[] = $latestKill['victim'][
    'ship_type_id'];
    ...
    if (!empty($idsToResolve)) {
       ...
       //gpi-services.co.uk/combatIntel/combatintel.html)'
            ...
            if ($namesResponse !== FALSE && $httpCode === 200) {
                ...
                error_log("Failed to resolve names for a chunk. HTTP Code: {$httpCode}. cURL Error:
       {$curlError}. ESI Response: {$namesResponse}");
